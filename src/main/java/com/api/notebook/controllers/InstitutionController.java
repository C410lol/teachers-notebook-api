...
//    }
    ...
    @PutMapping("/{institutionId}/edit")
    @PreAuthorize("hasRole('ROLE_SUPER')")
    public ResponseEntity<?> editInstitution(
            @PathVariable(value = "institutionId") UUID institutionId,
        ...
        BeanUtils.copyProperties(institutionDto, institutionOptional.get());
        institutionService.save(institutionOptional.get());

        return ResponseEntity.ok("Instituição editada com sucesso!");
    }
}
